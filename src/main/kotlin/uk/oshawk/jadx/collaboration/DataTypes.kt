...
class NodeRef(
    ...
    override fun compareTo(other: NodeRef) = compareValuesBy(this, other, { it.type }, { it.declaringClass }, { it.shortId })
...
class ProjectRename(val identifier: Identifier, val newName: String) : Comparable<ProjectRename> {
    constructor(rename: ICodeRename) : this(Identifier(NodeRef(rename.nodeRef), rename.codeRef?.let { CodeRef(it) }), rename.newName)
